...
def overwrite(fname, cont1, cont2):
    file = open('AC/' + fname, 'ab')
    file.seek(0)
    file.truncate()
    file.write(cont1)
    file.close()

    file1 = open('BD/' + fname, 'ab')
    file1.seek(0)
    file1.truncate()
    file1.write(cont2)
    file1.close()
...
def write_file(fname, cont1, cont2):
    with open('AC/' + fname, 'wb') as file:
        file.write(cont1)
    with open('BD/' + fname, 'wb') as file1:
        file1.write(cont2)

def read_file(name):
    with open('AC/'+ name, 'rb') as file:
        cont1 = file.read()
    with open('BD/'+ name, 'rb') as file:
        cont2 = file.read()
    return cont1 + cont2
